...
// Descripción: Cliente de resultados con diseño en cards, brújula política, matches/conflictos y análisis detallado.
  ...
  <CardContent>
    <div className="relative h-96 w-full border-2 border-border">
      ...
      {results.map((result) => (
        <div
          key={result.party.id}
          className="absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-muted-foreground"
          style={{
            left: `${((Number(result.party.economic_axis || 0) + 10) / 20) * 100}%`,
            top: `${((10 - Number(result.party.social_axis || 0)) / 20) * 100}%`,
          }}
